...
try: from gmpy2 import mpz; mpzv, inttypes =      2, (int, int, type(mpz(1)))
except ImportError:
 try: from gmpy import mpz; mpzv, inttypes =      1, (int, int, type(mpz(1)))
 except ImportError:   mpz, mpzv, inttypes = int, 0, (int, int)
    ...
    while True:
        if n not in sieve:
            if n < q: yield n
                ...
                vnext, step = q + 2*p, 2*p
                while vnext in sieve: vnext += step
                sieve[vnext] = step
                p = next(ps)
                q = p**2
        else:
